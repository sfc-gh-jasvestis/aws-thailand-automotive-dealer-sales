...
export default function HomePage() {
  ...
  const executiveCockpit = (
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KPICard title="Dealer Revenue (MTD)" value={kpiVal('Dealer Revenue (MTD)', '฿2.8B')} status="neutral" />
        <KPICard title="Inventory Days" value={kpiVal('Inventory Days', '42')} status="warning" />
        <KPICard title="Lead Conversion" value={kpiVal('Lead Conversion', '18.7%')} status="neutral" />
        <KPICard title="Active Dealers" value={kpiVal('Active Dealers', '312')} status="neutral" />
      </div>
  ...
  const domainTab1 = (
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <KPICard title="EV Mix Forecast" value={kpiVal('EV Mix Forecast', '34%')} />
        <KPICard title="Avg Days to Sale" value={kpiVal('Avg Days to Sale', '28')} />
        <KPICard title="Test Drive Conversion" value={kpiVal('Test Drive Conversion', '24%')} />
      </div>
      <Chart
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <Chart
          ...
          type="pie"
          xKey="label"
          yKeys={[{ key: 'value', name: 'Achievement %' }]}
          title="Performance vs Target"
        />
        <ActionMemo
          persona={{ name: 'Anuwat Prasertsri', role: 'VP Dealer Operations' }}
          context={{}}
          onGenerate={async () => {
            const memos = [
              {
                ...
                actions: ['Launch incentive for underperforming Central dealers', 'Reallocate EV inventory to Eastern region', 'Schedule dealer training for hybrid launch'],
              },
              {
  ...
  const askAiTab = (
    <div className="h-[600px]">
      <AskAI
        title="Ask AI"
        sampleQuestions={[
          'Which dealers are below 80% of quarterly target?',
  ...
  const tabs = [
    ...
    { id: 'domain-2', label: 'Dealer Intelligence', icon: '⚡', content: domainTab2 },
